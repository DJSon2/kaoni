...
def fetch_and_store_job_postings(api_url):
    ...
    for item in data['data']:
        ...
        kind = item['채용직종']
        enddate = item['공고종료일']
        organization = item['기관명']
        # regdate 처리, 값이 없으면 현재 시간으로 설정
        regdate = item.get('regdate', current_time).strip() if item.get('regdate', '').strip() else current_time
        content = f"근무지 - {item['근무지']}, 전형방법 - {item['전형방법']}, 채용방법 - {item['채용방법']}"
            
        # 데이터베이스에 삽입
        insert_job_posting(seq, title, kind, enddate, organization, content, regdate)

# API 엔드포인트 URL - 실제 요청 URL을 사용합니다.
api_url = 'https://api.odcloud.kr/api/15025788/v1/uddi:1d5e4090-c95e-42b6-b4b1-b4e2f2453f1a?page=1&perPage=10&serviceKey=bfzIrXZ6samznW7lltLbfU8XyvtHTbku5Q9zjQZQm0%2FTSyawXf2D3O091EYgnILM%2FzSBQxhqkeYqIwwa7srQeQ%3D%3D'
